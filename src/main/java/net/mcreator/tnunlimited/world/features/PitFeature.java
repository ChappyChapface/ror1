...
public class PitFeature extends Feature<NoneFeatureConfiguration> {
	public static PitFeature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new PitFeature();
		CONFIGURED_FEATURE = FeatureUtils.register("tnunlimited:pit", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("tnunlimited:pit", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("tnunlimited", "pit"));
		if (template == null)
			return false;
		...
		if ((context.random().nextInt(1000000) + 1) <= 2000) {
			...
			for (int a = 0; a < count; a++) {
				...
				BlockPos spawnTo = new BlockPos(i + 5, j + -8, k + 12);
