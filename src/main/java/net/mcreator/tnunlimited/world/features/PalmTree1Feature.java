...
public class PalmTree1Feature extends Feature<NoneFeatureConfiguration> {
	public static PalmTree1Feature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new PalmTree1Feature();
		CONFIGURED_FEATURE = FeatureUtils.register("tnunlimited:palm_tree_1", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("tnunlimited:palm_tree_1", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("tnunlimited", "palmtree1"));
		if (template == null)
			return false;
		...
		if ((context.random().nextInt(1000000) + 1) <= 150000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (!base_blocks.contains(context.level().getBlockState(new BlockPos(i, j, k)).getBlock()))
					continue;
				BlockPos spawnTo = new BlockPos(i + 7, j + 0, k + 6);
