...
public class GlockRightClickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
		...
		if (itemstack.getOrCreateTag().getDouble("ammo") > 0) {
			...
			itemstack.getOrCreateTag().putDouble("gun_damage", 1);
			...
			if (itemstack.getOrCreateTag().getDouble("ammo") > 1) {
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("tnunlimited:revolver_shoot")), SoundSource.PLAYERS, (float) 0.25, 2);
					} else {
						_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("tnunlimited:revolver_shoot")), SoundSource.PLAYERS, (float) 0.25, 2, false);
					}
				}
			} else {
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("tnunlimited:revolver_last_shot")), SoundSource.PLAYERS, (float) 0.25, 2);
					} else {
						_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("tnunlimited:revolver_last_shot")), SoundSource.PLAYERS, (float) 0.25, 2, false);
					}
				}
			}
			...
			entity.getPersistentData().putDouble("pitchrecoil", (((Math.random() * 2 * (-1) + -4) * (1 + 0.25 * EnchantmentHelper.getItemEnchantmentLevel(TnunlimitedModEnchantments.CALIBER.get(), itemstack)))
					/ (1 + EnchantmentHelper.getItemEnchantmentLevel(TnunlimitedModEnchantments.SHARP_SHOOTER.get(), itemstack) * 0.25)));
			if (world instanceof ServerLevel _level)
				_level.sendParticles((SimpleParticleType) (TnunlimitedModParticleTypes.BULLET_SHELL.get()), (entity.getX()), (entity.getY() + 1), (entity.getZ()), 1, 0.2, 0.2, 0.2, 0.1);
			if (!(new Object() {
			...
			itemstack.getOrCreateTag().putDouble("clip_size", 15);
