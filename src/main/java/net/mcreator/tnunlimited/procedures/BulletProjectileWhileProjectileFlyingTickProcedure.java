...
public class BulletProjectileWhileProjectileFlyingTickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity immediatesourceentity) {
		...
		if (0 == immediatesourceentity.getPersistentData().getDouble("maxDistance")) {
			...
			if (TnunlimitedModItems.REVOLVER.get() == (entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem()) {
				immediatesourceentity.getPersistentData().putDouble("maxDistance", 5);
			} else if (TnunlimitedModItems.FLINTLOCK.get() == (entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem()) {
				immediatesourceentity.getPersistentData().putDouble("maxDistance", 7);
			} else if (TnunlimitedModItems.ANTIMONY_STEN.get() == (entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem()) {
				immediatesourceentity.getPersistentData().putDouble("maxDistance", 2);
			} else if (TnunlimitedModItems.BLUNDERBUSS.get() == (entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem()) {
				immediatesourceentity.getPersistentData().putDouble("maxDistance", 2);
			} else if (TnunlimitedModItems.NUBRIUM_PISTOL.get() == (entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem()) {
