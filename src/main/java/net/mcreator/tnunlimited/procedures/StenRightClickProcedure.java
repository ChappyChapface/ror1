	...
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
		...
		if (itemstack.getOrCreateTag().getDouble("ammo") > 0) {
			...
			if (entity instanceof Player _player)
				_player.getCooldowns().addCooldown(itemstack.getItem(), (int) (3 / (1 + EnchantmentHelper.getItemEnchantmentLevel(TnunlimitedModEnchantments.TRIGGER_HAPPY.get(), itemstack) * 0.25)));
			itemstack.getOrCreateTag().putDouble("gun_damage", 0.5);
			...
			if (itemstack.getOrCreateTag().getDouble("ammo") > 1) {
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("tnunlimited:sten_shoot")), SoundSource.PLAYERS, 1, 1);
					} else {
						_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("tnunlimited:sten_shoot")), SoundSource.PLAYERS, 1, 1, false);
					}
				}
			} else {
				if (world instanceof Level _level) {
			...
			if (world instanceof ServerLevel _level)
				_level.sendParticles((SimpleParticleType) (TnunlimitedModParticleTypes.BULLET_SHELL.get()), (entity.getX()), (entity.getY() + 1), (entity.getZ()), 1, 0.2, 0.2, 0.2, 0.3);
			...
			entity.getPersistentData().putDouble("yawrecoil", (((Math.random() * 7 - 2) * (1 + 0.25 * EnchantmentHelper.getItemEnchantmentLevel(TnunlimitedModEnchantments.CALIBER.get(), itemstack)))
					/ (1 + EnchantmentHelper.getItemEnchantmentLevel(TnunlimitedModEnchantments.SHARP_SHOOTER.get(), itemstack) * 0.25)));
			entity.getPersistentData().putDouble("pitchrecoil", (((Math.random() * 3 * (-1) + 1) * (1 + 0.25 * EnchantmentHelper.getItemEnchantmentLevel(TnunlimitedModEnchantments.CALIBER.get(), itemstack)))
					/ (1 + EnchantmentHelper.getItemEnchantmentLevel(TnunlimitedModEnchantments.SHARP_SHOOTER.get(), itemstack) * 0.25)));
			if (!(new Object() {
			...
			itemstack.getOrCreateTag().putDouble("clip_size", 32);
