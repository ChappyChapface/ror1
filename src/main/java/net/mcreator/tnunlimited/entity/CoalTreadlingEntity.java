...
public class CoalTreadlingEntity extends Monster {

	public CoalTreadlingEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(TnunlimitedModEntities.COAL_TREADLING.get(), world);
	}

	public CoalTreadlingEntity(EntityType<CoalTreadlingEntity> type, Level world) {
		super(type, world);
		maxUpStep = 0.6f;
		xpReward = 5;
		setNoAi(false);
	...
	public static void init() {
		SpawnPlacements.register(TnunlimitedModEntities.COAL_TREADLING.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
			return
