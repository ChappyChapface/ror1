...
public class FrostAntEntity extends Monster implements IAnimatable {
	...
	public static final EntityDataAccessor<String> ANIMATION = SynchedEntityData.defineId(FrostAntEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<String> TEXTURE = SynchedEntityData.defineId(FrostAntEntity.class, EntityDataSerializers.STRING);
	...
	public FrostAntEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(TnunlimitedModEntities.FROST_ANT.get(), world);
	}

	public FrostAntEntity(EntityType<FrostAntEntity> type, Level world) {
		super(type, world);
		xpReward = 8;
		setNoAi(false);
	}
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "frost_ant");
	}
	...
	public static void init() {
		SpawnPlacements.register(TnunlimitedModEntities.FROST_ANT.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
	...
	private <E extends IAnimatable> PlayState movementPredicate(AnimationEvent<E> event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.frost_ant.walk", EDefaultLoopTypes.LOOP));
				return PlayState.CONTINUE;
			}
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.frost_ant.idle", EDefaultLoopTypes.LOOP));
			return PlayState.CONTINUE;
		}
